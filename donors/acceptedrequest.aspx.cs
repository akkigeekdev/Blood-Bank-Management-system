...
public partial class donors_acceptedrequest : System.Web.UI.Page
{
    ...
    public void bindgrid()
    {
        ...
        con.Open();
        SqlCommand com = new SqlCommand("SELECT R.id, B.bg, D.name, R.req_date FROM request AS R INNER JOIN donors AS D ON D.id = R.donorid INNER JOIN bloodbank AS B ON B.id = R.bg WHERE (R.status = 1) AND D.ID = " + Session["User"].ToString());
        com.Connection = con;
